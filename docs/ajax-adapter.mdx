...
Related: `AjaxProxy.for(scriptInclude, defaults?)`, `AjaxProxy.channel(scriptInclude, method, { debounce, latest, timeout, retry })`, `AjaxProxy.setDefaultTimeout(ms)`, `AjaxProxy.setErrorHandler(fn)`, `AjaxProxy.setLogTable(name)`, `AjaxProxy.setDigestIntegration(enabled)`, `AjaxProxy.logUrl(errorOrReference)`.
